...
**Status: APPROVED AND LOCKED.** The V4 homepage — promoted from `/v4` to the real homepage at `/` — is `src/app/page.tsx` and everything it composes from `src/components/v4/`. It is the primary visual and behavioural source of truth for the Pixelocracy website going forward. This document extracts the system from that approved implementation — it does not invent a new one, and it does not redesign anything. Where this document and the live code ever disagree, **the code is right and this document is stale** — fix the document. `/v4` now permanently redirects to `/` (see `next.config.ts`); there is only one editable copy of the homepage.
